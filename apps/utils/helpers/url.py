...
class UrlBuilder(object):
    def __init__(self, domain='', path='', page='', secure=False, shorten_link=False, keep_trailing_slash=True, params=None):
        self._domain = domain
    ...
    def build(self):
        ...
        if self._params:
            for param in self._params:
                if isinstance(param, dict):
                    ...
                    if len(param) > 2:
                        raise Exception(
                            'Tuple must follow structure (key, value)')
                    if len(param) == 1:
                        raise Exception(
        ...
        if extracted_params:
            params_str = '?%s' % (
                urlencode(extracted_params, quote_via=quote_plus))
...
def _make_request(method='GET', url=None, params=None, data=None, **kwargs):
    try:
